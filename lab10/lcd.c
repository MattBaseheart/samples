...
*/
void lcd_writecommand(unsigned char cmd) //DO THIS
{
...
*/
void lcd_writedata(unsigned char dat) //DO THIS
{
...
*/
void lcd_writenibble(unsigned char lcdbits) //DO THIS
{
    /* Load PORTD, bits 7-4 with bits 7-4 of "lcdbits" */
	PORTD &= ~(MASKBITS); //clear PORTD bits 7-4
	PORTD |= (lcdbits & MASKBITS); //force where 1s are, pass where 0s are


    /* Make E signal (PB1) go to 1 and back to 0 */
	PINB = 0x02;
	PINB = 0x02;
	PORTB &= ~(0x02); //force pin 1 to 0
